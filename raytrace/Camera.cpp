...
void Camera::set(const float3& eye_point, const float3& view_point, const float3& up_vector, float camera_constant)
{
	...
	//(1.2) setting up pinhole camera.
	//calculate basis for imageplane to world space
	ip_normal = normalize(lookat - eye); //z
	ip_xaxis = normalize(cross(ip_normal, up_vector));//x
	ip_yaxis = cross(ip_xaxis, ip_normal);//y
...
float3 Camera::get_ray_dir(const float2& coords) const
{
	...
	//(1.2) pinhole. get ray direction in the new basis
